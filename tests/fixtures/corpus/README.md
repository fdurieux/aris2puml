...
|------|--------|------|-------|
| `mortgage-application.json` | mortgage origination: application check, credit history, approval, property inspection, deed, mortgage insurance, disbursement | 11 f / 17 e / 7 xor / 1 and / 1 or, 41 edges | its rework loop is now a `backward`; still refused — the XOR join before *Reject loan application* is fed by outcomes of two different XOR splits (unstructured); behind it, the AND split after approval joins at an OR (kind mismatch) |
| `mortgage-application-variant.json` | the same process modelled by a second author, with a rework loop and a disconnected fragment | 13 f / 18 e / 6 xor / 1 and / 1 or, 42 edges | its rework loop is now a `while`; still refused — the XOR join before *Approve loan application* is fed by outcomes of two different XOR splits (unstructured); behind it, the AND split after approval joins at an OR (kind mismatch) |
...
The one left was a loop whose exit outcome has no event, emitted as
`repeat while (…) is (E) not` with a bare `not`; the same check found six
loops whose *back* edge has no event, emitted as `is not (X)`, which
PlantUML accepts and mis-parses (the condition swallows the text). Since
2026-09-06 each half of the tail is written only when the loop has that
event, and **all 1 994 render**. The SAP set has no org units at all, so
